...
function PropertiesPage(router: Router): HTMLElement {
  const page = el('div');
...
function PropertyDetailPage(title: string, property: Property): HTMLElement {
  const page = el('div');
  const sec = el('section', 'sec');
...
(function start() {
  ...
  // Mount router outlet into main
  main.appendChild(router['outlet'] as HTMLElement);
  router.render();
})();